...
- K 线图：K 线、6 条可分别设置周期和 SMA/EMA 的均线、VWAP、MACD、RSI、KDJ、布林带、成交量、Auto Fibonacci 和 60d 筹码峰。Auto Fib Retracement 以 Deviation 和 Depth 自动识别最近波段，Auto Fib Extension 仅以独立的 Depth 自动寻找 A-B-C 结构；最新发展中锚点会随新 K 线移动。参数位于 Indicator settings，调整只重算当前已加载的 OHLCV，不会重新下载行情。VWAP 仅累计有成交量的 K 线，日内图按交易日重置；单用户版将参数保留在当前浏览器会话，多用户版登录后会按账号保存。
...
- K-line charts: candlesticks, six independently configurable SMA/EMA moving averages, VWAP, MACD, RSI, KDJ, Bollinger Bands, volume, Auto Fibonacci, and a 60d volume-by-price profile. Auto Fib Retracement uses Deviation and Depth to find the latest swing, while Auto Fib Extension uses its own Depth to find an A-B-C structure. A developing endpoint can move as new bars arrive; this expected repainting keeps the levels current. Controls live in Indicator settings and only recalculate the OHLCV already loaded in the chart—no market-data request is made. VWAP uses only bars with volume and resets by trading day for intraday charts. Settings are kept for the browser session in the single-user app and persisted per account in the multi-user app.
- Short-term Watchlist (multi-user): an account-scoped list with separate 5-minute and 15-minute tables. It shows the latest-bar change, 15-bar inline SVG candle and MACD/signal sparklines, two configurable MA distances, MACD difference, Bollinger-upper position, and session-reset VWAP distance. Its 10/20/30-second refresh is independent of the normal stock auto-refresh.
